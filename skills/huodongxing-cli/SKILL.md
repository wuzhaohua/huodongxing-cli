...
description: "使用安全 CLI 操作活动行（huodongxing.com）：搜索活动、读取结构化详情、生成推荐、执行报名预检，并为票券、主办方、名单、营销和财务页面解析登录态路由。用户提到活动行、查找活动、活动报名、主办方后台或活动运营时使用。"
---
...
3. 活动发现、详情、推荐和报名预检使用公开命令。
4. 登录态页面先运行 `open-route --print-only` 获取精确 URL，再使用当前 Agent 可用的浏览器工具读取。若没有浏览器工具，只向用户返回 URL，不声称已经读取页面。
5. 任何写操作先运行 `action-plan`，向用户展示准确对象、范围、个人数据、费用和可逆性。
6. 只有在用户明确确认本次具体动作后，才通过真实浏览器完成最终提交；提交前重新读取页面，提交后验证最终状态。
