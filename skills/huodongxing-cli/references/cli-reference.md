...
要求 Python 3.9+。公共命令仅使用标准库。登录态页面由 `open-route --print-only` 解析 URL，再由当前 Agent 可用的浏览器能力读取和操作。
...
默认城市为 `全国`。`--page` 和 `--limit` 必须是大于等于 1 的整数。自定义日期必须同时提供 `--from` 和 `--to`，使用 `YYYY-MM-DD` 格式，并且开始日期不得晚于结束日期。
...
`--print-only` 只输出精确 URL，适合无界面 Agent 和沙箱。省略该参数时，CLI 调用系统默认浏览器。CLI 不读取 Cookie 或私有接口；Agent 通过自身可用的浏览器能力读取登录态页面，默认不要输出手机号、邮箱、身份证或完整名单。
...
JSON 用于 Agent 和程序；CSV 用于表格；Markdown 用于报告。搜索和推荐 CSV 每个活动一行，嵌套对象编码成 JSON 字符串。
